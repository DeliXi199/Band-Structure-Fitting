...
!-----------------------------------------------------------!
   ...
   subroutine read_txt_file()
        ...
        integer :: i, j, k, nx, ny, nz, Nband_Fermi_Level, Nk_v, a, dim_size
        real(dp) , dimension(3,3) :: lattice_matrix


        ! 读取 NMS.txt 文件
        open(unit=1, file='NMS.txt', status='old', action='read')
        ...
        open(unit=2, file='star_R.txt', status='old', action='read')
        do i = 1, nx
            do j = 1, ny
        ...
        open(unit=3, file='star_R_len.txt', status='old', action='read')
        do i = 1, nx
        ...
        nx = NMS(2)
        ! 分配并读取 coes 数组
        allocate(coes(NMS(4),nx))
        open(unit=4, file='coes.txt', status='old', action='read')
        do i = 1, NMS(4)
        ...
        close(4)
         ...
         Nk_v=8841
        allocate(fitk4v(Nk_v,3))
        open(unit=5, file='star_R_copy.txt', status='old', action='read')
        do i = 1, Nk_v
            read(5, *) (fitk4v(i,j), j = 1, 3) ! 从文件中逐行读取
         end do
         close(5)

         dim_size = NMS(4)
         allocate(my_bands(dim_size))
         open(unit=6, file='my_bands.txt', status='old', action='read')
         read(6, *) my_bands 
         close(6)
   ...
   subroutine get_mykv(iband,Nband_Fermi_Level,bands_fermi_level)
      ...
      real(dp), allocatable, dimension(:,:) ::vkk
      real(dp), allocatable, dimension(:) ::E
      real(dp), dimension(3) :: fitk4vi
      real(dp), dimension(3) :: vkki
      real(dp) :: Ei
      len = size(fitk4v,1)
      allocate(vkk(len,3))
      allocate(E(len))
      ...
      do i=1,len
         fitk4vi = fitk4v(i,:)
         ! print *, fitk4vi
         ! call my_E((1),fitk4vi,Ei)
         ! call E_calc_myband(Nband_Fermi_Level, bands_fermi_level, fitk4vi, Ei)
         ! call velocity_calc_myband(Nband_Fermi_Level, bands_fermi_level, fitk4vi, vkki)
         call my_velocity(1, fitk4vi, vkki)
         ! print *, vkki

         ! print *, vkki
      end do

      open(unit=10, file='vkk.txt', status='replace')
      do i = 1, len
         write(10, *) vkk(i,:)  ! 写入文件
      end do
      close(10)

   end subroutine get_mykv


   subroutine velocity_calc_myband(Nband_Fermi_Level, bands_fermi_level, k, velocity_k)
      ...
      real(dp) :: velocity_k(3, Nband_Fermi_Level)
      real(dp) :: Ek(Nband_Fermi_Level) 
      ...
      return
   end subroutine velocity_calc_myband

   subroutine E_calc_myband(Nband_Fermi_Level, bands_fermi_level, k, Ei)
      ...
      Ei = W(bands_fermi_level(1))

      return
   end subroutine E_calc_myband
   ...
   subroutine velocity_calc(Nband_Fermi_Level, bands_fermi_level, k, velocity_k, Ek)
      ...
      if (is_wannier) then
         call velocity_calc_wannier(Nband_Fermi_Level, bands_fermi_level, k, velocity_k, Ek)
      else
         call velocity_calc_fourier(Nband_Fermi_Level, bands_fermi_level, k, velocity_k, Ek)
      endif
   end subroutine velocity_calc


   subroutine velocity_calc_fourier(Nband_Fermi_Level, bands_fermi_level, k, velocity_k, Ek)
      ...
      real(dp), intent(inout) :: velocity_k(3, Nband_Fermi_Level)
      real(dp), intent(out) :: Ek(Nband_Fermi_Level) 
      real(8), dimension(3) :: velocity_vector       ! 速度向量
      integer :: iband, iiband
      real(8) :: Eki

      do iband = 1, Nband_Fermi_Level
         call find_position(bands_fermi_level(iband),iiband)
         call my_velocity(iiband, k, velocity_vector)  ! 调用速度计算函数
         velocity_k(:,iband) =  velocity_vector
         call my_E(iiband, k, Eki)  ! 调用能量计算函数
         Ek(iband) = Eki
      end do

      return
   end subroutine velocity_calc_fourier
   ...
   ! 计算速度的函数
    subroutine my_velocity(iband,kkk, velocity_vector)
        ...
        real(8), dimension(3,3) :: lattice_matrix
        real(8) :: a
        ...
        do m = 1, NMS(2)
            ...
            do l = 1, star_R_len(m)
                v_m = v_m - sin(dot_product(star_R_m(l, :), kkk)*pi*2) * star_R_m(l, :)  ! 累积计算速度
            end do
            v = v + coe(m) * v_m / star_R_len(m) ! 更新速度向量
        end do
         ...
         ! 定义 FCC 晶格矢量
         lattice_matrix = reshape([ 0.0d0, a/2.0d0, a/2.0d0,  &
                                    a/2.0d0,   0.0d0, a/2.0d0,  &
                                    a/2.0d0, a/2.0d0,   0.0d0 ], [3,3])

        velocity_vector = matmul(v,lattice_matrix)
    ...
    subroutine my_E(iband,kpointsk,Eki)
        ...
        do m = 1, NMS(2)
            ...
            E_m = 0.0d0 ! 初始化临时速度向量
            do l = 1, star_R_len(m)
                E_m = E_m + cos(dot_product(star_R_m(l, :), kpointsk)*pi*2)   ! 累积计算速度
            end do
    ...
    subroutine find_position(iband, iiband)
      use wmpi
      use para

      implicit none
      integer, intent(in) :: iband               ! 输入数字
      integer, intent(out) :: iiband         ! 输出位置

      integer :: i

      iiband = 1  ! 默认未找到，位置为 1

      ! 遍历数组，查找数字的位置
      do i = 1, NMS(4)
         if (my_bands(i) == iband) then
               iiband = i
               return
         end if
      end do
   ...
   subroutine velocity_calc_wannier(Nband_Fermi_Level, bands_fermi_level, k, velocity_k, Ek)
      ...
      return
   end subroutine velocity_calc_wannier
   ...
   subroutine velocity_bar_calc(magnetic_field, Nband_Fermi_Level, bands_fermi_level, Btau, k0, velocity_bar_k)
      ...
      if (is_wannier) then
         call velocity_bar_calc_wannier(magnetic_field, Nband_Fermi_Level, bands_fermi_level, Btau, k0, velocity_bar_k)
      else
         call velocity_bar_calc_fourier(magnetic_field, Nband_Fermi_Level, bands_fermi_level, Btau, k0, velocity_bar_k)
      end if
   
   end subroutine velocity_bar_calc

   subroutine velocity_bar_calc_fourier(magnetic_field, Nband_Fermi_Level, bands_fermi_level, Btau, k0, velocity_bar_k)
      ...
      allocate(W(Nband_Fermi_Level))
      allocate(kout(3, NSlice_Btau))
      allocate(velocity_init(Nband_Fermi_Level, 3, NSlice_Btau))


      do i = 1, Nband_Fermi_Level
         call find_position(bands_fermi_level(i), iiband)
         call my_E(iiband, k, Wi)  ! 调用能量计算函数
         W(i)= Wi
      end do
      ...
      return
   end subroutine velocity_bar_calc_fourier


   !> calculate averaged velocity for given k points and a given band index
   !> Eq.(3) in PRB 79, 245123(2009)
   subroutine velocity_bar_calc_wannier(magnetic_field, Nband_Fermi_Level, bands_fermi_level, Btau, k0, velocity_bar_k)
      ...
      return
   end subroutine velocity_bar_calc_wannier
   ...
   subroutine velocity_calc_iband2(iband, k, velocity_k, E_iband)
      ...
      if (is_wannier) then
         call velocity_calc_iband2_wannier(iband, k, velocity_k, E_iband)
      else
         call velocity_calc_iband2_fourier(iband, k, velocity_k, E_iband)
      end if
   ...
   !> calculate velocity for given k points for all bands in bands_fermi_level
   !> Eq.(2) in PRB 79, 245123(2009)
   subroutine velocity_calc_iband2_wannier(iband, k, velocity_k, E_iband)
      ...
      return
   end subroutine velocity_calc_iband2_wannier



   subroutine velocity_calc_iband2_fourier(iband, k, velocity_k, E_iband)
      ...
      call find_position(iband, iiband)

      call my_E(iiband, k, E_iband)  ! 调用能量计算函数

      if (abs(E_iband)/eV2Hartree>EF_integral_range) then
         velocity_k= 0d0
         return
      endif

      call my_velocity(iiband, k, velocity_k)  ! 调用速度计算函数

   end subroutine velocity_calc_iband2_fourier
   ...
   subroutine velocity_calc_iband(iband, k, velocity_k)
      ...
      if (is_wannier) then
         call velocity_calc_iband_wannier(iband, k, velocity_k)
      else
         call velocity_calc_iband_fourier(iband, k, velocity_k)
      end if

      return
   end subroutine velocity_calc_iband

   subroutine velocity_calc_iband_fourier(iband, k, velocity_k)
   ...
   end subroutine velocity_calc_iband_fourier



   !> calculate velocity for given k points for all bands in bands_fermi_level
   !> Eq.(2) in PRB 79, 245123(2009)
   subroutine velocity_calc_iband_wannier(iband, k, velocity_k)
...
!     UU= Hamk_bulk(:, iband)
      ...
      return
   end subroutine velocity_calc_iband_wannier
